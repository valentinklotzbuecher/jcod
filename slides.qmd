...
- Sharp, known treatment timing, constant measurement

**Not the right tool:**

- Patient-level RCT 
- Many treated units with staggered timing → staggered DiD
- Fuzzy timing, anticipation effects
...
- **Synthetic Difference-in-Differences (SDID):** Combines SCM-style unit weights with DiD-style time weights — reweights both donors and pre-periods [@ArkhangelskyEtAl2021SDID]

- **Augmented SCM:** Adds ridge regression bias correction on top of SCM weights — reduces interpolation bias when treated unit lies outside donor convex hull [@BenMichaelEtAl2021AugSCM]

- **Interactive Fixed Effects (IFE) / Matrix Completion (MC):** Models outcome as low-rank factor structure and imputes missing counterfactual via matrix completion [@Xu2017gsynth; @AtheyEtAl2021MatrixCompletion]

---

## Software

- `scpi`: canonical SCM + inference/prediction intervals [@CattaneoEtAl2025scpi]
- `synthdid`, `augsynth`: SDID and augmented SCM
...
[![](https://api.qrserver.com/v1/create-qr-code/?size=320x320&data=https%3A%2F%2Fvalentink.quarto.pub%2Fjcod-scm%2F){width=280}](https://valentink.quarto.pub/jcod-scm/)

Slides & resources: [valentink.quarto.pub/jcod-scm](https://valentink.quarto.pub/jcod-scm)
